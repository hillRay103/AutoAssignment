...
export async function buildDocx(data: any, outPath: string): Promise<void> {
    ...
    // ══ TEMPLATE C ═══════════════════════════════════════════════════════════
        ...
        children.push(new Table({
            ...
            rows: [new TableRow({ children: [
                ...
                new TableCell({
                    children: [P([R(fullTitle, { bold:true, size:14, color:WHITE })], { sb:2, sa:2 })],
                    shading: { type:ShadingType.CLEAR, fill:BLUE, color:'auto' },
                    width: { size:8960, type:WidthType.DXA },
                    borders: { top:noB(), bottom:noB(), left:noB(), right:noB() },
                    margins: { top:0, bottom:0, left:160, right:120 },
                }),
            ]})],
        }));
        children.push(P([], { sa:3 }));
        const parts = [courseStr, section?`Section ${section}`:'', instructor].filter(Boolean);
        children.push(P([R(`${name}  |  ${reg}`, { bold:true, size:11, color:NAVY })], { align:AlignmentType.CENTER, sa:1 }));
        children.push(P([R(parts.join('  |  '),  { size:10, color:MGRAY })],            { align:AlignmentType.CENTER, sa:1 }));
        children.push(P([R(`Date: ${subDate}`,   { size:10, color:MGRAY })],            { align:AlignmentType.CENTER, sa:3 }));
    ...
    // ══ TEMPLATE D ═══════════════════════════════════════════════════════════
        ...
        const side: any[] = [];
        if (clo) {
            side.push(new Table({ width:{size:100,type:WidthType.PERCENTAGE}, rows:[new TableRow({ children:[new TableCell({
                children:[P([R(clo,{bold:true,size:10,color:WHITE})],{sb:1,sa:1})],
        ...
        side.push(P([R(course, { bold:true, size:22, color:DGRAY })], { sa:2 }));
        if (assignTtl) side.push(P([R(assignTtl, { size:12, color:MGRAY, italic:true })], { sa:8 }));
        side.push(divLine(NAVY, 6));
        if (project) {
            side.push(P([R('Project Title:', { bold:true, size:11, color:NAVY })], { sb:6, sa:2 }));
            side.push(P([R(project, { bold:true, italic:true, size:13, color:NAVY })], { sa:10 }));
            side.push(divLine(NAVY, 4));
        }
        for (const [txt,b,col] of [
            [`Submitted to: ${instructor}`,true,MGRAY],
            ['SUBMITTED BY:',true,NAVY],
            [`${reg}  |  ${name.toUpperCase()}`,true,BLUE],
            [`Date: ${subDate}`,false,MGRAY],
        ] as [string,boolean,string][]) {
            side.push(P([R(txt,{bold:b,size:10,color:col})],{align:AlignmentType.RIGHT,sa:2}));
        }
        children.push(new Table({
            width: { size:100, type:WidthType.PERCENTAGE },
            layout: TableLayoutType.FIXED,
            rows: [new TableRow({ children:[
                new TableCell({
                    children:[P([],{})],
                    shading:{type:ShadingType.CLEAR,fill:NAVY,color:'auto'},
                    width:{size:convertInchesToTwip(0.28),type:WidthType.DXA},
                    borders:{top:noB(),bottom:noB(),left:noB(),right:noB()},
                }),
                new TableCell({
                    children:side,
                    width:{size:convertInchesToTwip(6.22),type:WidthType.DXA},
                    borders:{top:noB(),bottom:noB(),left:noB(),right:noB()},
...
export async function buildPdf(data: any, outPath: string): Promise<void> {
    ...
    if (tpl === 'A') {
        ...
        fillR(LM, y, 16, 36, NAVY);
        fillR(LM+16, y, CW-16, 36, BLUE);
        dt(fullTitle, LM+28, y-24, { bold:true, size:13, color:WHITE });
        y-=42;
        const parts=[courseStr, section?`Section ${section}`:'', instructor].filter(Boolean);
        y = tl(`${name}  |  ${reg}`, LM, y, { bold:true, size:10, color:NAVY, align:'center', width:CW, gap:2 });
        y = tl(parts.join('  |  '),  LM, y, { size:9, color:MGRAY, align:'center', width:CW, gap:2 });
        y = tl(`Date: ${subDate}`,   LM, y, { size:9, color:MGRAY, align:'center', width:CW, gap:4 });
        hLine(y, BLUE, 1.5); y-=8;

    } else if (tpl === 'D') {
        fillR(LM, y, 16, 500, NAVY);
        const rx=LM+24, rw=CW-24;
        if (clo) { fillR(rx,y,130,20,'1A6B8A'); dt(clo,rx+6,y-14,{bold:true,size:9,color:WHITE}); y-=28; }
        y = tl(course, rx, y, { bold:true, size:18, color:DGRAY, width:rw, gap:4 });
        if (assignTtl) { y = tl(assignTtl, rx, y, { italic:true, size:11, color:MGRAY, width:rw, gap:6 }); }
        hLine(y,NAVY,1.5); y-=8;
        if (project) {
            y = tl('Project Title:', rx, y, { bold:true, size:10, color:NAVY, width:rw, gap:3 });
            y = tl(project, rx, y, { bold:true, italic:true, size:12, color:NAVY, width:rw, gap:8 });
            hLine(y,NAVY,1); y-=8;
        }
        for (const [t,b,col] of [
            [`Submitted to: ${instructor}`,true,MGRAY],
            ['SUBMITTED BY:',true,NAVY],
            [`${reg}  |  ${name.toUpperCase()}`,true,BLUE],
            [`Date: ${subDate}`,false,MGRAY],
        ] as [string,boolean,string][]) {
            const f=b?fB:fN, sz=9;
            dt(t, rx+rw-f.widthOfTextAtSize(t,sz), y-sz, {bold:b,size:sz,color:col});
            y-=sz+5;
        }
        newPage();
    }
